...
fn generation_advances_only_when_the_index_changes() {
    ...
    fs::write(&target, "namespace alpha { void run() {} void added() {} }\n").unwrap();
    touch_future(&target);

    let after = run_cx(p.path(), &["--json", "symbols", "--all"]);
    assert_eq!(generation(&after), 2, "{}", after.stdout);
    assert_eq!(after.json()["freshness"]["files_updated"].as_u64().unwrap(), 1);

    let settled = run_cx(p.path(), &["--json", "symbols", "--all"]);
    assert_eq!(generation(&settled), 2, "no further bump: {}", settled.stdout);
}
...
fn same_size_same_mtime_edit_is_missed_by_metadata_and_caught_by_verified() {
    ...
    let stale = run_cx(p.path(), &["--json", "symbols", "--file", "src/scope_a.cpp", "--all"]);
    ...
    assert_eq!(stale.json()["freshness"]["mode"].as_str().unwrap(), "metadata");
    assert_eq!(stale.json()["freshness"]["files_updated"].as_u64().unwrap(), 0);
    let stale_generation = generation(&stale);

    // verified mode hashes contents and picks it up.
    let fresh = run_cx(
        p.path(),
        &["--fresh", "verified", "--json", "symbols", "--file", "src/scope_a.cpp", "--all"],
    );
    ...
    assert_eq!(fresh.json()["freshness"]["mode"].as_str().unwrap(), "verified");
    assert_eq!(fresh.json()["freshness"]["files_updated"].as_u64().unwrap(), 1);
    assert_eq!(
        generation(&fresh),
        stale_generation + 1,
...
fn refresh_paths_catches_a_same_size_same_mtime_edit() {
    ...
    assert_eq!(refreshed.json()["freshness"]["mode"].as_str().unwrap(), "paths");
    assert_eq!(refreshed.json()["freshness"]["files_checked"].as_u64().unwrap(), 1);

    // The edit is now in the index, provably in the generation refresh reported.
    let refreshed_generation = generation(&refreshed);
    let query = run_cx(p.path(), &["--json", "symbols", "--file", "src/scope_a.cpp", "--all"]);
    assert_eq!(
        generation(&query),
...
fn content_change_with_same_size_but_new_mtime_is_caught_by_metadata() {
    ...
    let out = run_cx(p.path(), &["--json", "symbols", "--file", "src/scope_a.cpp", "--all"]);
    let names: Vec<String> = out
        .results()
        .iter()
        .map(|r| r["name"].as_str().unwrap().to_string())
        .collect();
    assert!(names.contains(&"jog".to_string()), "mtime changed, so metadata suffices: {names:?}");
}
...
fn refresh_reports_per_path_status() {
    ...
    fs::write(&edited, "namespace alpha { void run() {} void more() {} }\n").unwrap();
    touch_future(&edited);
...
fn refresh_ignores_paths_outside_the_project() {
    ...
    assert_eq!(out.json()["freshness"]["files_updated"].as_u64().unwrap(), 0);
}
...
fn refresh_without_paths_verifies_the_whole_project() {
    ...
    assert_eq!(out.code, 0, "stderr: {}", out.stderr);
    assert_eq!(out.json()["freshness"]["mode"].as_str().unwrap(), "verified");
    let rows = out.results();
    assert_eq!(rows.len(), 1, "only the changed file is reported: {}", out.stdout);
    assert_eq!(rows[0]["file"].as_str().unwrap(), "src/scope_a.cpp");
    assert_eq!(rows[0]["status"].as_str().unwrap(), "updated");
}
...
fn new_deleted_and_renamed_files_are_counted_in_freshness() {
    ...
    assert_eq!(out.json()["freshness"]["files_updated"].as_u64().unwrap(), 1);
    assert_eq!(out.json()["freshness"]["files_removed"].as_u64().unwrap(), 0);

    // Deleted file.
    fs::remove_file(&added).unwrap();
    let out = run_cx(p.path(), &["--json", "symbols", "--all"]);
    assert_eq!(out.json()["freshness"]["files_updated"].as_u64().unwrap(), 0);
    assert_eq!(out.json()["freshness"]["files_removed"].as_u64().unwrap(), 1);
...
fn concurrent_readers_agree_on_one_generation() {
    ...
    let handles: Vec<_> = (0..4)
        .map(|_| {
            let root = root.clone();
            std::thread::spawn(move || run_cx(&root, &["--json", "symbols", "--name", "run", "--all"]))
        })
        .collect();
...
fn a_writer_and_readers_do_not_corrupt_the_index() {
    ...
    let target = p.path().join("src/scope_a.cpp");
    fs::write(&target, "namespace alpha { void run() {} void concurrent() {} }\n").unwrap();
    touch_future(&target);
    ...
    let after = run_cx(p.path(), &["--json", "symbols", "--name", "concurrent", "--all"]);
